...
### Phase B7 — Food actually enters the nest (added 2026-07-26, first slice landed)

A forager reaching the nest marker used to have its load evaporate into a single colony-wide number. Food never went underground at all — so the underground view showed a colony that ate nothing, and the store chambers the plan was digging had nothing to put in them.

- [x] **Entrance moved to the centre of the surface view** (top middle of the cross-section). Asked for to make tunnel patterns legible, and it turned out to be a real architectural improvement, not a cosmetic one: the corner placement meant the shaft had a wall immediately to one side so it could only zig-zag one way; chambers hung on the wall side were sized by the world edge instead of by their depth; half the spoil crater was off-world; and diggers recruited against the wall got carried along it by `avoidSurfaces`' hugging. Chamber count at pop 900 went **7 → 13**, because rooms can now hang off *both* sides of the shaft — which is what real nests do (measured nests run 1 to 4-5 parallel vertical chamber series). `NEST_CORNER_MARGIN` is gone.
- [x] **`STATE_STORE` + new `provisioning.js`.** A delivery pause at the nest no longer ends the trip: the ant keeps its load, walks to the entrance, crosses under, carries it to a chamber, and hands it over there. The handover pause is at the chamber, because that's where the food actually changes hands.
- [x] **Chambers hold food** (`chamber.food`, capacity from floor area via `CHAMBER_FOOD_PER_AREA`), drawn as a scatter of morsels so the larder visibly fills.
- [x] **Where food goes follows the biology**: food goes where it is CONSUMED. A store chamber is a *preference* — used if the colony has one with room — and the fallback is the deepest chamber, where the brood and queen live. Most ants don't warehouse food in rooms at all; dedicated granaries are a granivore speciality, so this must not be modeled as the default.
- Verified: food reaches chambers (3241 of 3777 units delivered into rooms over 2400 simulated seconds), no burial, other suites unaffected. The ~13% shortfall is loads credited to the colony on a give-up timeout — deliberately credited rather than destroyed, same reasoning as a dropped spoil pellet, but it's a number to watch.

Follow-ups landed the same day, from watching it run in the browser:
- **No pause at the entrance.** A returning forager used to stop at the nest marker for a dropoff pause — a leftover from when that was where food entered the colony. It isn't any more, so the ant now walks straight in and the only pause is the handover at the chamber. Also improved delivery reliability sharply (food reaching a chamber went from 86% to 99.9% of colony intake), since loads no longer sit at the door long enough to hit give-up timeouts.
- **The entrance is a hole, not a mound.** `NEST_DRAW_RADIUS` 20 -> 8: a real nest entrance is barely wider than the ants using it, and what makes an established nest look big from above is the crater of spoil around it, which now builds up on its own. `NEST_ARRIVE_RADIUS` had to be decoupled from it first — it was `NEST_DRAW_RADIUS + ANT_LENGTH`, which silently tied *arrival precision* to how big the entrance was *drawn*, so shrinking the marker would have tightened the approach on every forager and digger and driven up timeouts.
- **The crater is drawn as a smooth rim** rather than 24 hard wedges (the bins are a model of a continuous rim, not physical sectors — as wedges it looked like a pie chart). Measured evenness is already slightly better than random (cv 0.51 against a Poisson ~0.58 at that pellet count), so the residual unevenness is small-sample scatter, which is also what real spoil heaps look like — they're frequently piled to one side.

**Entrance and crossing polish (2026-07-26, all from watching it in the browser):**
- **Ants cross at the centre of the visible hole**, not at a radius around it. The crossing threshold used to be `NEST_DRAW_RADIUS + ANT_LENGTH`, so shrinking the marker to a realistic hole made ants wink out over open sand; then at the rim. Now `ENTRANCE_CROSS_RADIUS` is 4px and they land on the middle (measured: 2.9px mean, 4.0px max from centre).
- **Both crossings happen off-screen underground.** An ant leaving climbs above the top edge of the cross-section before it transfers, and one arriving is placed above the edge and walks down into frame — so the shaft mouth just looks like a way out. The underground entrance dot was removed; the mouth is self-evidently the exit, and the dot marked a spot where nothing happens.
- **The air gap above the top edge is only at the shaft mouth.** Exempting the whole edge from the dirt clamp let ants that had climbed out drift sideways over packed earth and sink in, putting burial back to 1.9% after it had been driven to zero.
- **Ants no longer orbit the entrance.** An ant cannot turn tighter than speed/steer-rate (~13px at a walk), so it physically could not land on a target a few pixels across. Three fixes were tried: a positional pull inward (works, looks like suction), slowing on approach (works, reads as hesitating at its own door — the user's call to drop it was right), and finally **sharpening the turn**, which shrinks the same radius at full speed. Median time to cross the final 30px is now 0.68s against a 0.67s theoretical straight-line walk, i.e. no orbit at all. Worth noting the fix had to go in `foraging.js`: a returning forager is in `STATE_RETURN` for the entire approach and only becomes `STATE_STORE` on arrival, so putting it only in the tasks that cross into the nest left the actual offenders untouched.
- **Dropping spoil takes a moment.** Haulers used to deposit instantly, so the trip read as pop out, pellet appears, straight back down. Also fixed `SPOIL_INNER_RADIUS`, which was a hardcoded 12 and ended up *inside* the entrance hole when the marker grew to 13 — every dump site sat on the entrance, so haulers emerged already standing on their target. Derived from `NEST_DRAW_RADIUS` now.

**Not done, and the honest gap: real foragers don't walk their load to the bottom of the nest.** They unload near the entrance to a receiver worker, and the food travels down through a chain of mouth-to-mouth handoffs (trophallaxis), not one ant's journey. Adults can't even eat solid food — only larvae can, and in many species workers place solids *on* the larvae, which externally digest them and are drunk from later, making the brood the colony's communal gut. Modeling the relay needs a receiver task and food that lives in ants rather than in rooms; sequenced with Phase E (nursing), where there'll finally be brood to feed.

**Known consequence of holding off on consumption:** nothing eats yet, so the deepest chamber becomes an ever-growing food dump and storage demand grows without bound. Phase C (the queen spending food on eggs) is the fix.

### Underground pheromones — considered and declined (2026-07-26)

Asked whether the surface trail system should extend underground. Recommendation: **no**, and not as a copy of the surface system.

- Trail pheromones are unquestionably real and general, and trails do run to and from nest entrances — but the coordination mechanism documented *inside* nests is not a trail grid. The excavation literature is specific that it's **stigmergy via deposited soil pellets** (a pellet dropped near a face recruits more diggers there) and **local density/collision cues** (an ant infers available space from how often it collides). The "Agitated ants" work makes the point explicitly: incipient excavation self-organizes through collisional cues, with no chemical signal required.
- Beware sources claiming things like "spiral harvester nests are coordinated by circular pheromone trails" — that shows up in low-quality aggregator content and is not in Tschinkel's actual casting work, which is the primary source for this project's nest architecture.
- Functionally it would also buy nothing here: underground ants navigate to *known* targets (a claimed cell, a specific chamber, the entrance) by straight-line seek. A trail solves a discovery problem, and there is no discovery problem in a nest you dug yourself.

What IS worth building is the pellet stigmergy, which is already Phase B4's aggregation bullet. Revisit a genuine in-nest chemical signal only if a later phase creates something an ant must *find* rather than be told about.
...
### Phase C — Queen + egg-laying — done (2026-07-27)
- Chamber placement was already solved for both: `nestPlan.js` hands back the deepest chamber and the brood-stratum rooms by purpose, so the queen and brood had real, queryable locations to be placed in rather than an abstract nest point.
- [x] **A queen**, as a plain object in `colony.js` — not in `ants.js`'s SoA store, since she has none of a worker's machinery and one object costs nothing. Her *behavior* lives in a new `queen.js`, the same store/behavior split `ants.js`/`behaviors.js` already use; `colony.js` stays a store.
- [x] **Eggs cost food.** `QUEEN_EGG_FOOD_COST` (2, i.e. two forager loads) comes out of the food physically sitting in chambers via `takeNestFood()`, with `colony.food` mirrored down. No food, no eggs — the first real stewardship pressure in the sim, and the first thing ever to spend the resource.
- [x] **Brood as plain objects in a plain array** (`colony.brood`), each an `{x, y}` in the room it was laid in. Phase D adds the stage and timer.
- [x] **The brood demand rule reads brood**, replacing the worker-population stand-in (`BROOD_AREA_PER_ANT` → `BROOD_AREA_PER_BROOD`). Consequence, and it looks right in practice: a new colony has no brood, so it digs its top rooms first and opens nursery strata only as the queen fills them.
- [x] **She walks.** Her chamber is a *position* (the deepest one), and the colony can dig below her, so when the bottom of the nest moves she relocates to it along `routeWaypoint()` at `QUEEN_SPEED` rather than teleporting — a pop on an always-on ambient screen is exactly the kind of thing you'd notice. Measured: 114px in ~9.5s, zero ticks standing in dirt over 5 trials from the shaft mouth and 5 from her chamber's edge. Ferrying the *brood* down with her still needs workers to carry it (Phase D/E).
- Two design points worth keeping:
  - **The nursery is not just her room.** `chooseBroodChamber()` prefers the room she's in, then any other brood-stratum chamber with space. That's a stand-in for nurses lifting eggs off her (real workers do this almost as fast as she lays), and without it she stops laying the instant her one room fills no matter how much nursery the colony has dug.
  - **`BROOD_AREA_PER_BROOD` (26) is deliberately larger than `BROOD_PACK_AREA` (16).** Demand per item of brood has to exceed how tightly brood actually packs, or a full nursery generates no demand for more nursery and the colony can never dig its way out of the stall.
- **Brood doesn't follow the nest down, and you can now see it.** Purpose is derived from depth, so when the colony digs below the founding chamber that room stops being nursery — but the eggs already in it stay where they were laid, because nothing can carry brood yet. Observed in a 75-minute run: the queen relocated twice (y=120 → 161 → 195) and left ~130 eggs piled in a room by then labelled "stores". This is the deferred migration item under Phase B2 (occupants moving down with the bottom third), and it needs Phase D/E workers to do the ferrying. Demand accounting is unaffected — brood outside the stratum still counts toward demand, and `chooseBroodChamber()` counts per-room by position, so nothing is double-counted.
- **What this does and does not fix.** The open loop it closes is real — food now has a sink and the deepest chamber is no longer a larder that only grows. But a queen laying one egg per ~35s eats ~3.4 food/minute, and a heavily-fed colony brings in far more than that, so `colony.food` (and with it store demand) still grows under generous feeding. The rest of the sink is Phase D/E: larvae eat, and there are many more of them than there are queens.
