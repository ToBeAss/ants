...
- [ ] **Movement through tunnels**: `avoidTunnelWalls()` (in `underground.js`) is built — mirrors `avoidSurfaces()`'s sum-push-vectors-first pattern, but sources "surfaces" from nearby `DIRT` cells instead of an obstacle list. **Not yet called from `sim.js`** — no ant currently has a reason to be in the underground domain (see entrance linkage below), so there's nothing to steer yet. Will get its first real caller once the digging task exists.
- [ ] **Entrance linkage**: `ants.domain` (`DOMAIN_SURFACE`/`DOMAIN_UNDERGROUND`, in `ants.js`) plus `enterUnderground()`/`exitToSurface()` (in `underground.js`) are built and swap-and-pop-safe. **Not yet called by anything** — nothing currently decides an ant should cross. First real caller is the digging task.
...
- [ ] **Digging task**: new worker state `DIG` (same state-machine extension pattern as `NURSE` in Phase E), likely in a new `digging.js`. Diggers work the frontier of the tunnel network, converting adjacent dirt cells to tunnel over time, bounded by the currently unlocked region (see containment framing above). **This is the first real consumer of movement-through-tunnels and entrance-linkage above** — building it will be what actually exercises both, not just unit-level verification.
...
- [ ] **Container expansion interaction**: new player action that grows the unlocked/diggable region (e.g. click a control, or click just outside the current boundary). `expandUnlockedRegion()` (in `underground.js`) is ready for this to call; not yet wired to input. Analogous to `placeObstacle` in `main.js`'s input handling.
- [x] **Rendering**: `undergroundRender.js` draws dirt vs. tunnel cells, the entrance marker, and the unlocked-region boundary (visualizes the current "container size"). Queen/brood-in-chamber and digging-ants-at-the-frontier visuals are deferred until those entities exist (Phases C/D and the digging task above) — the underground-ant draw loop is in place and correct, just currently empty since no ant is ever in that domain yet.
