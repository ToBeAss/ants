...
- [ ] **Underground data model**: a new file (e.g. `underground.js`) owning a 2D grid of cells, each dirt (solid) or tunnel (dug out) — the side-view cross-section. Same "one file, one concern" convention as everywhere else; don't fold this into `world.js`.
- [ ] **Movement through tunnels**: reuse the project's existing continuous-steering paradigm rather than building a discrete tunnel-graph pathfinder — treat dirt cells as blocking mass the same way `avoidSurfaces()` already treats obstacles/walls, so tunnel-dwelling ants steer through open (dug) cells and are blocked by solid ones. Keeps faith with the project's established "no pathfinding" philosophy instead of introducing a second, inconsistent navigation model.
- [ ] **Entrance linkage**: the existing top-down nest marker's position becomes the single entry point connecting the two views. An ant crossing it disappears from one view and appears in the other at the corresponding point — no need for simultaneous dual-rendering.
- [ ] **View switching**: single canvas, toggle between surface and underground views on a keypress (same pattern as the existing `T` trail toggle, different key since `T` is taken). Noted for later: the two rendering paths should stay cleanly separable (independent draw functions/canvases, not intertwined) so that a future dual-*screen* setup — one physical screen per view, each oriented to match its view angle, closer to a real terrarium — is a natural extension rather than a rearchitecture. Confirmed as the eventual ideal, toggle is the deliberate near-term scope.
- [ ] **Digging task**: new worker state `DIG` (same state-machine extension pattern as `NURSE` in Phase E), likely in a new `digging.js`. Diggers work the frontier of the tunnel network, converting adjacent dirt cells to tunnel over time.
- [ ] **Initial chamber**: a small starting chamber dug out at the entrance point at init, so the queen (Phase C) and first brood (Phase D) have somewhere to be from the start rather than needing digging to complete first.
- [ ] **Rendering**: new underground draw path (likely `undergroundRender.js`, keeping `render.js` focused on the surface view or as the dispatcher between the two) — dirt vs. tunnel cells, the queen/brood in their chamber, digging ants at the frontier.
